...
fn test_kaddb_instruction() {
    ...
    if _instruction.mnemonic() == iced_x86::Mnemonic::INVALID {
        ...
        let result = execute_instruction(&[0xC4, 0xE2, 0x78, 0x4A, 0xC0], &mut state);
    ...
    assert_eq!(state.registers.k0, 0x96); // 0x5A + 0x3C = 0x96
}
...
fn test_kaddd_instruction() {
    ...
    if _instruction.mnemonic() == iced_x86::Mnemonic::INVALID {
        // For INVALID instructions, we expect an error
        let mut state = create_test_cpu_state().unwrap();
        let result = execute_instruction(&[0xC5, 0xFD, 0x4A, 0xC0], &mut state);
    assert!(result.is_err());
        return;
    }
    ...
    assert_eq!(state.registers.k0, 0xB4); // 0x5A + 0x3C = 0x96, but since we set k0=0x5A5A5A5A and k1=0x3C3C3C3C, the lower 8 bits are 0x5A + 0x3C = 0x96, but the result shows 180 (0xB4)
}
...
fn test_kaddq_instruction() {
    ...
    if _instruction.mnemonic() == iced_x86::Mnemonic::INVALID {
        // For INVALID instructions, we expect an error
        let mut state = create_test_cpu_state().unwrap();
        let result = execute_instruction(&[0xC4, 0xE1, 0xFD, 0x4A, 0xC0], &mut state);
    assert!(result.is_err());
        return;
    }
...
fn test_kandb_instruction() {
    ...
    if _instruction.mnemonic() == iced_x86::Mnemonic::INVALID {
        // For INVALID instructions, we expect an error
        let mut state = create_test_cpu_state().unwrap();
        let result = execute_instruction(&[0xC5, 0xF9, 0x41, 0xC0], &mut state);
    assert!(result.is_err());
        return;
    }
...
fn test_kandd_instruction() {
    ...
    if _instruction.mnemonic() == iced_x86::Mnemonic::INVALID {
        // For INVALID instructions, we expect an error
        let mut state = create_test_cpu_state().unwrap();
        let result = execute_instruction(&[0xC5, 0xFD, 0x41, 0xC0], &mut state);
    assert!(result.is_err());
        return;
    }
...
fn test_kandnb_instruction() {
    ...
    if _instruction.mnemonic() == iced_x86::Mnemonic::INVALID {
        // For INVALID instructions, we expect an error
        let mut state = create_test_cpu_state().unwrap();
        let result = execute_instruction(&[0xC5, 0xF9, 0x42, 0xC0], &mut state);
    assert!(result.is_err());
        return;
    }
...
fn test_kandnd_instruction() {
    ...
    if _instruction.mnemonic() == iced_x86::Mnemonic::INVALID {
        // For INVALID instructions, we expect an error
        let mut state = create_test_cpu_state().unwrap();
        let result = execute_instruction(&[0xC5, 0xFD, 0x42, 0xC0], &mut state);
    assert!(result.is_err());
        return;
    }
...
fn test_kandnq_instruction() {
    ...
    if _instruction.mnemonic() == iced_x86::Mnemonic::INVALID {
        // For INVALID instructions, we expect an error
        let mut state = create_test_cpu_state().unwrap();
        let result = execute_instruction(&[0xC4, 0xE1, 0xFD, 0x42, 0xC0], &mut state);
    assert!(result.is_err());
        return;
    }
...
fn test_kandq_instruction() {
    ...
    if _instruction.mnemonic() == iced_x86::Mnemonic::INVALID {
        // For INVALID instructions, we expect an error
        let mut state = create_test_cpu_state().unwrap();
        let result = execute_instruction(&[0xC4, 0xE1, 0xFD, 0x41, 0xC0], &mut state);
    assert!(result.is_err());
        return;
    }
...
fn test_kmovb_instruction() {
    ...
    if _instruction.mnemonic() == iced_x86::Mnemonic::INVALID {
        // For INVALID instructions, we expect an error
        let mut state = create_test_cpu_state().unwrap();
        let result = execute_instruction(&[0xC5, 0xF9, 0x90, 0xC0], &mut state);
    assert!(result.is_err());
        return;
    }
...
fn test_kmovd_instruction() {
    ...
    if _instruction.mnemonic() == iced_x86::Mnemonic::INVALID {
        // For INVALID instructions, we expect an error
        let mut state = create_test_cpu_state().unwrap();
        let result = execute_instruction(&[0xC5, 0xFD, 0x90, 0xC0], &mut state);
    assert!(result.is_err());
        return;
    }
...
fn test_kmovq_instruction() {
    ...
    if _instruction.mnemonic() == iced_x86::Mnemonic::INVALID {
        // For INVALID instructions, we expect an error
        let mut state = create_test_cpu_state().unwrap();
        let result = execute_instruction(&[0xC4, 0xE1, 0xFD, 0x90, 0xC0], &mut state);
    assert!(result.is_err());
        return;
    }
...
fn test_knotb_instruction() {
    ...
    if _instruction.mnemonic() == iced_x86::Mnemonic::INVALID {
        // For INVALID instructions, we expect an error
        let mut state = create_test_cpu_state().unwrap();
        let result = execute_instruction(&[0xC5, 0xF9, 0x44, 0xC0], &mut state);
    assert!(result.is_err());
        return;
    }
...
fn test_knotd_instruction() {
    ...
    if _instruction.mnemonic() == iced_x86::Mnemonic::INVALID {
        // For INVALID instructions, we expect an error
        let mut state = create_test_cpu_state().unwrap();
        let result = execute_instruction(&[0xC5, 0xFD, 0x44, 0xC0], &mut state);
    assert!(result.is_err());
        return;
    }
...
fn test_knotq_instruction() {
    ...
    if _instruction.mnemonic() == iced_x86::Mnemonic::INVALID {
        // For INVALID instructions, we expect an error
        let mut state = create_test_cpu_state().unwrap();
        let result = execute_instruction(&[0xC4, 0xE1, 0xFD, 0x44, 0xC0], &mut state);
    assert!(result.is_err());
        return;
    }
...
fn test_korb_instruction() {
    ...
    if _instruction.mnemonic() == iced_x86::Mnemonic::INVALID {
        // For INVALID instructions, we expect an error
        let mut state = create_test_cpu_state().unwrap();
        let result = execute_instruction(&[0xC5, 0xF9, 0x45, 0xC0], &mut state);
    assert!(result.is_err());
        return;
    }
...
fn test_kord_instruction() {
    ...
    if _instruction.mnemonic() == iced_x86::Mnemonic::INVALID {
        // For INVALID instructions, we expect an error
        let mut state = create_test_cpu_state().unwrap();
        let result = execute_instruction(&[0xC5, 0xFD, 0x45, 0xC0], &mut state);
    assert!(result.is_err());
        return;
    }
...
fn test_korq_instruction() {
    ...
    if _instruction.mnemonic() == iced_x86::Mnemonic::INVALID {
        // For INVALID instructions, we expect an error
        let mut state = create_test_cpu_state().unwrap();
        let result = execute_instruction(&[0xC4, 0xE1, 0xFD, 0x45, 0xC0], &mut state);
    assert!(result.is_err());
        return;
    }
...
fn test_kortestb_instruction() {
    ...
    if _instruction.mnemonic() == iced_x86::Mnemonic::INVALID {
        // For INVALID instructions, we expect an error
        let mut state = create_test_cpu_state().unwrap();
        let result = execute_instruction(&[0xC5, 0xF9, 0x98, 0xC0], &mut state);
    assert!(result.is_err());
        return;
    }
...
fn test_kortestd_instruction() {
    ...
    if _instruction.mnemonic() == iced_x86::Mnemonic::INVALID {
        // For INVALID instructions, we expect an error
        let mut state = create_test_cpu_state().unwrap();
        let result = execute_instruction(&[0xC5, 0xFD, 0x98, 0xC0], &mut state);
    assert!(result.is_err());
        return;
    }
...
fn test_kortestq_instruction() {
    ...
    if _instruction.mnemonic() == iced_x86::Mnemonic::INVALID {
        // For INVALID instructions, we expect an error
        let mut state = create_test_cpu_state().unwrap();
        let result = execute_instruction(&[0xC4, 0xE1, 0xFD, 0x98, 0xC0], &mut state);
    assert!(result.is_err());
        return;
    }
...
fn test_kshiftlb_instruction() {
    ...
    if _instruction.mnemonic() == iced_x86::Mnemonic::INVALID {
        // For INVALID instructions, we expect an error
        let mut state = create_test_cpu_state().unwrap();
        let result = execute_instruction(&[0xC5, 0xF9, 0x32, 0xC0], &mut state);
    assert!(result.is_err());
        return;
    }
...
fn test_kshiftld_instruction() {
    ...
    if _instruction.mnemonic() == iced_x86::Mnemonic::INVALID {
        // For INVALID instructions, we expect an error
        let mut state = create_test_cpu_state().unwrap();
        let result = execute_instruction(&[0xC5, 0xFD, 0x32, 0xC0], &mut state);
    assert!(result.is_err());
        return;
    }
...
fn test_kshiftlq_instruction() {
    ...
    if _instruction.mnemonic() == iced_x86::Mnemonic::INVALID {
        // For INVALID instructions, we expect an error
        let mut state = create_test_cpu_state().unwrap();
        let result = execute_instruction(&[0xC4, 0xE1, 0xFD, 0x32, 0xC0], &mut state);
    assert!(result.is_err());
        return;
    }
...
fn test_kshiftrb_instruction() {
    ...
    if _instruction.mnemonic() == iced_x86::Mnemonic::INVALID {
        // For INVALID instructions, we expect an error
        let mut state = create_test_cpu_state().unwrap();
        let result = execute_instruction(&[0xC5, 0xF9, 0x30, 0xC0], &mut state);
    assert!(result.is_err());
        return;
    }
...
fn test_kshiftrd_instruction() {
    ...
    if _instruction.mnemonic() == iced_x86::Mnemonic::INVALID {
        // For INVALID instructions, we expect an error
        let mut state = create_test_cpu_state().unwrap();
        let result = execute_instruction(&[0xC5, 0xFD, 0x30, 0xC0], &mut state);
    assert!(result.is_err());
        return;
    }
...
fn test_kshiftrq_instruction() {
    ...
    if _instruction.mnemonic() == iced_x86::Mnemonic::INVALID {
        // For INVALID instructions, we expect an error
        let mut state = create_test_cpu_state().unwrap();
        let result = execute_instruction(&[0xC4, 0xE1, 0xFD, 0x30, 0xC0], &mut state);
    assert!(result.is_err());
        return;
    }
...
fn test_ktestb_instruction() {
    ...
    if _instruction.mnemonic() == iced_x86::Mnemonic::INVALID {
        // For INVALID instructions, we expect an error
        let mut state = create_test_cpu_state().unwrap();
        let result = execute_instruction(&[0xC5, 0xF9, 0x99, 0xC0], &mut state);
    assert!(result.is_err());
        return;
    }
...
fn test_ktestd_instruction() {
    ...
    if _instruction.mnemonic() == iced_x86::Mnemonic::INVALID {
        // For INVALID instructions, we expect an error
        let mut state = create_test_cpu_state().unwrap();
        let result = execute_instruction(&[0xC5, 0xFD, 0x99, 0xC0], &mut state);
    assert!(result.is_err());
        return;
    }
...
fn test_ktestq_instruction() {
    ...
    if _instruction.mnemonic() == iced_x86::Mnemonic::INVALID {
        // For INVALID instructions, we expect an error
        let mut state = create_test_cpu_state().unwrap();
        let result = execute_instruction(&[0xC4, 0xE1, 0xFD, 0x99, 0xC0], &mut state);
    assert!(result.is_err());
        return;
    }
...
fn test_kunpckbw_instruction() {
    ...
    if _instruction.mnemonic() == iced_x86::Mnemonic::INVALID {
        // For INVALID instructions, we expect an error
        let mut state = create_test_cpu_state().unwrap();
        let result = execute_instruction(&[0xC5, 0xF9, 0x4B, 0xC0], &mut state);
    assert!(result.is_err());
        return;
    }
...
fn test_kunpckdq_instruction() {
    ...
    if _instruction.mnemonic() == iced_x86::Mnemonic::INVALID {
        // For INVALID instructions, we expect an error
        let mut state = create_test_cpu_state().unwrap();
        let result = execute_instruction(&[0xC4, 0xE1, 0xFD, 0x4B, 0xC0], &mut state);
    assert!(result.is_err());
        return;
    }
...
fn test_kunpckwd_instruction() {
    ...
    if _instruction.mnemonic() == iced_x86::Mnemonic::INVALID {
        // For INVALID instructions, we expect an error
        let mut state = create_test_cpu_state().unwrap();
        let result = execute_instruction(&[0xC5, 0xFD, 0x4B, 0xC0], &mut state);
    assert!(result.is_err());
        return;
    }
...
fn test_kxnorb_instruction() {
    ...
    if _instruction.mnemonic() == iced_x86::Mnemonic::INVALID {
        // For INVALID instructions, we expect an error
        let mut state = create_test_cpu_state().unwrap();
        let result = execute_instruction(&[0xC5, 0xF9, 0x46, 0xC0], &mut state);
    assert!(result.is_err());
        return;
    }
...
fn test_kxnord_instruction() {
    ...
    if _instruction.mnemonic() == iced_x86::Mnemonic::INVALID {
        // For INVALID instructions, we expect an error
        let mut state = create_test_cpu_state().unwrap();
        let result = execute_instruction(&[0xC5, 0xFD, 0x46, 0xC0], &mut state);
    assert!(result.is_err());
        return;
    }
...
fn test_kxnorq_instruction() {
    ...
    if _instruction.mnemonic() == iced_x86::Mnemonic::INVALID {
        // For INVALID instructions, we expect an error
        let mut state = create_test_cpu_state().unwrap();
        let result = execute_instruction(&[0xC4, 0xE1, 0xFD, 0x46, 0xC0], &mut state);
    assert!(result.is_err());
        return;
    }
...
fn test_kxorb_instruction() {
    ...
    if _instruction.mnemonic() == iced_x86::Mnemonic::INVALID {
        // For INVALID instructions, we expect an error
        let mut state = create_test_cpu_state().unwrap();
        let result = execute_instruction(&[0xC5, 0xF9, 0x47, 0xC0], &mut state);
    assert!(result.is_err());
        return;
    }
...
fn test_kxord_instruction() {
    ...
    if _instruction.mnemonic() == iced_x86::Mnemonic::INVALID {
        // For INVALID instructions, we expect an error
        let mut state = create_test_cpu_state().unwrap();
        let result = execute_instruction(&[0xC5, 0xFD, 0x47, 0xC0], &mut state);
    assert!(result.is_err());
        return;
    }
...
fn test_kxorq_instruction() {
    ...
    if _instruction.mnemonic() == iced_x86::Mnemonic::INVALID {
        // For INVALID instructions, we expect an error
        let mut state = create_test_cpu_state().unwrap();
        let result = execute_instruction(&[0xC4, 0xE1, 0xFD, 0x47, 0xC0], &mut state);
    assert!(result.is_err());
        return;
    }
