...
fn test_mov_instruction() {
    ...
    let result = execute_instruction(&[0x48, 0x89, 0xD8], &mut state); // MOV RBX, RAX
    
    println!("After: RAX = 0x{:X}, RBX = 0x{:X}", state.registers.rax, state.registers.rbx);
    
    assert_eq!(state.registers.rax, 0x123456789ABCDEF0); // RAX unchanged
    assert_eq!(state.registers.rbx, 0x123456789ABCDEF0); // RBX gets RAX's value
}
...
fn test_mov_immediate() {
    ...
    let result = execute_instruction(&[0x48, 0xC7, 0xC0, 0x78, 0x56, 0x34, 0x12], &mut state); // MOV RAX, 0x12345678
...
fn test_mov_memory_to_register() {
    ...
    let result = execute_instruction(&[0x48, 0x8B, 0x04, 0x25, 0x00, 0x10, 0x00, 0x00], &mut state); // MOV RAX, [0x1000]
...
fn test_mov_register_to_memory() {
    ...
    let result = execute_instruction(&[0x48, 0x89, 0x04, 0x25, 0x00, 0x10, 0x00, 0x00], &mut state); // MOV [0x1000], RAX
...
fn test_movsb_instruction() {
    ...
    let result = execute_instruction(&[0xA4], &mut state); // MOVSB
    assert_eq!(state.read_u8(0x2000).unwrap(), 0xAB);
    assert_eq!(state.registers.rsi, 0x1001);
    assert_eq!(state.registers.rdi, 0x2001);
}
...
fn test_movsb_backward() {
    ...
    let result = execute_instruction(&[0xA4], &mut state); // MOVSB
    assert_eq!(state.read_u8(0x2001).unwrap(), 0xCD);
    assert_eq!(state.registers.rsi, 0x1000);
    assert_eq!(state.registers.rdi, 0x2000);
}
...
fn test_instruction_error_handling() {
    ...
    let mut state = create_test_cpu_state().unwrap();
    let result = execute_instruction(&[0xFF, 0xFF, 0xFF], &mut state);
