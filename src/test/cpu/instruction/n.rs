...
fn create_test_cpu_state() -> Result<CpuState> {
    let memory = GuestMemory::new(0x100000)?; // 1MB of memory
...
mod tests {
    ...
    fn test_nop_instruction() {
        ...
        let result = execute_instruction(&[0x90], state).unwrap();
        
        // NOP should not change any registers
        assert_eq!(result.registers.rax, initial_rax);
    }
    ...
    fn test_nop_instruction_multiple() {
        ...
        let result = execute_instruction(&[0x90, 0x90, 0x90], state).unwrap();
        
        // NOP should not change any registers
        assert_eq!(result.registers.rax, initial_rax);
        assert_eq!(result.registers.rbx, initial_rbx);
    }
    ...
    fn test_neg_instruction_register() {
        ...
        let result = execute_instruction(&[0x48, 0xF7, 0xD8], state).unwrap();
        
        // NEG 5 = -5 = 0xFFFFFFFFFFFFFFFB
        assert_eq!(result.registers.rax, 0xFFFFFFFFFFFFFFFB);
        
        // Debug: Check what flags are actually set
        println!("CARRY flag: {}", result.registers.get_flag(RFlags::CARRY));
        println!("OVERFLOW flag: {}", result.registers.get_flag(RFlags::OVERFLOW));
        println!("All flags: {:?}", result.registers.rflags);
        ...
        assert!(!result.registers.get_flag(RFlags::OVERFLOW)); // 5 is not the most negative value
    }
    ...
    fn test_neg_instruction_negative() {
        ...
        // NEG (-5) = 5
        assert_eq!(result.registers.rax, 5);
    ...
    fn test_neg_instruction_memory() {
        ...
        state.memory.write_u64(0x1000, 10).unwrap();
        
        // NEG QWORD PTR [RAX] (0x48, 0xF7, 0x18)
        let result = execute_instruction(&[0x48, 0xF7, 0x18], state).unwrap();
        
        // NEG 10 = -10 = 0xFFFFFFFFFFFFFFF6
        let value = result.memory.read_u64(0x1000).unwrap();
    ...
    fn test_not_instruction_register() {
        ...
        let result = execute_instruction(&[0x48, 0xF7, 0xD0], state).unwrap();
        
        // NOT should flip all bits
        assert_eq!(result.registers.rax, 0xEDCBA9876543210F);
    ...
    fn test_not_instruction_zero() {
        ...
        assert_eq!(result.registers.rax, 0xFFFFFFFFFFFFFFFF);
    ...
    fn test_not_instruction_memory() {
        let mut state = create_test_cpu_state().unwrap();
        state.registers.rax = 0x1000; // Memory address
        state.memory.write_u64(0x1000, 0x123456789ABCDEF0).unwrap();
        
        // NOT QWORD PTR [RAX] (0x48, 0xF7, 0x10)
        let result = execute_instruction(&[0x48, 0xF7, 0x10], state).unwrap();
    ...
    fn test_neg_instruction_32bit() {
        ...
        // NEG EAX (0xF7, 0xD8)
        let result = execute_instruction(&[0xF7, 0xD8], state).unwrap();
        
        // NEG 5 = -5 = 0xFFFFFFFB (32-bit result in lower 32 bits of RAX)
        assert_eq!(result.registers.rax & 0xFFFFFFFF, 0xFFFFFFFB);
    ...
    fn test_not_instruction_32bit() {
        ...
        // NOT EAX (0xF7, 0xD0)
        let result = execute_instruction(&[0xF7, 0xD0], state).unwrap();
        
        // NOT should flip all bits (32-bit result in lower 32 bits of RAX)
        assert_eq!(result.registers.rax & 0xFFFFFFFF, 0xEDCBA987);
    ...
    fn test_neg_instruction_16bit() {
        ...
        let result = execute_instruction(&[0x66, 0xF7, 0xD8], state).unwrap();
        
        // NEG 5 = -5 = 0xFFFB (16-bit result in lower 16 bits of RAX)
        assert_eq!(result.registers.rax & 0xFFFF, 0xFFFB);
    ...
    fn test_not_instruction_16bit() {
        ...
        let result = execute_instruction(&[0x66, 0xF7, 0xD0], state).unwrap();
        
        // NOT should flip all bits (16-bit result in lower 16 bits of RAX)
        assert_eq!(result.registers.rax & 0xFFFF, 0xEDCB);
    ...
    fn test_neg_instruction_8bit() {
        ...
        // NEG AL (0xF6, 0xD8)
        let result = execute_instruction(&[0xF6, 0xD8], state).unwrap();
        
        // NEG 5 = -5 = 0xFB (8-bit result in lower 8 bits of RAX)
        assert_eq!(result.registers.rax & 0xFF, 0xFB);
    ...
    fn test_not_instruction_8bit() {
        ...
        // NOT AL (0xF6, 0xD0)
        let result = execute_instruction(&[0xF6, 0xD0], state).unwrap();
        
        // NOT should flip all bits (8-bit result in lower 8 bits of RAX)
        assert_eq!(result.registers.rax & 0xFF, 0xED);
    ...
    fn test_nop_variants() {
        ...
        // Test different NOP variants
        // NOPW (0x66, 0x90) - 16-bit NOP
        let result1 = execute_instruction(&[0x66, 0x90], state.clone()).unwrap();
        assert_eq!(result1.registers.rax, initial_rax);
        
        // NOPL (0x90) - 32-bit NOP (same as regular NOP)
        let result2 = execute_instruction(&[0x90], state.clone()).unwrap();
        assert_eq!(result2.registers.rax, initial_rax);
        
        // NOPQ (0x48, 0x90) - 64-bit NOP
        let result3 = execute_instruction(&[0x48, 0x90], state).unwrap();
        assert_eq!(result3.registers.rax, initial_rax);
    }
    ...
    fn test_not_instruction_edge_cases() {
        ...
        // NOT should flip all bits
        assert_eq!(result.registers.rax, 0x5555555555555555);
    ...
    fn test_memory_boundary_conditions() {
        ...
        state.memory.write_u64(0xF0000, 10).unwrap();
        
        // NEG QWORD PTR [RAX] (0x48, 0xF7, 0x18)
        let result = execute_instruction(&[0x48, 0xF7, 0x18], state).unwrap();
        
        // Should work correctly
        let value = result.memory.read_u64(0xF0000).unwrap();
        assert_eq!(value, 0xFFFFFFFFFFFFFFF6);
    }
    ...
    fn test_neg_not_combination() {
        ...
        // First NEG: 5 -> -5
        let result1 = execute_instruction(&[0x48, 0xF7, 0xD8], state).unwrap();
        assert_eq!(result1.registers.rax, 0xFFFFFFFFFFFFFFFB);
        
        // Then NOT: -5 -> 4
        let result2 = execute_instruction(&[0x48, 0xF7, 0xD0], result1).unwrap();
        assert_eq!(result2.registers.rax, 4);
    }
    ...
    fn test_nop_instruction_sequence() {
        ...
        let result = execute_instruction(&[0x90, 0x90, 0x90, 0x90, 0x90], state).unwrap();
