...
fn create_test_cpu_state() -> Result<CpuState> {
    let memory = GuestMemory::new(1024 * 1024)?;
...
mod tests {
    ...
    fn test_or_instruction_register() {
        ...
        let result = execute_instruction(&[0x48, 0x09, 0xD8], state).unwrap();
        
        // OR result: 0x123456789ABCDEF0 | 0xFEDCBA9876543210 = 0xFEFCFEF8FEFCFEF0
        assert_eq!(result.registers.rax, 0xFEFCFEF8FEFCFEF0);
    ...
    fn test_or_instruction_memory() {
        ...
        let result = execute_instruction(&[0x48, 0x0B, 0x03], state).unwrap();
    ...
    fn test_or_instruction_8bit() {
        ...
        let result = execute_instruction(&[0x08, 0xD8], state).unwrap();
        
        // OR result: 0x5A | 0xA5 = 0xFF
        assert_eq!(result.registers.rax & 0xFF, 0xFF);
        assert!(!result.registers.get_flag(RFlags::ZERO));
        assert!(result.registers.get_flag(RFlags::SIGN));
    }
    ...
    fn test_or_instruction_16bit() {
        ...
        let result = execute_instruction(&[0x66, 0x09, 0xD8], state).unwrap();
        
        // OR result: 0xDE5A | 0x32A5 = 0xFEFF
        assert_eq!(result.registers.rax & 0xFFFF, 0xFEFF);
        assert!(!result.registers.get_flag(RFlags::ZERO));
        assert!(result.registers.get_flag(RFlags::SIGN));
    }
    ...
    fn test_or_instruction_32bit() {
        ...
        let result = execute_instruction(&[0x09, 0xD8], state).unwrap();
        
        // OR result: 0x9ABCDEF0 | 0x76543210 = 0xFEFCFEF0
        assert_eq!(result.registers.rax & 0xFFFFFFFF, 0xFEFCFEF0);
        assert!(!result.registers.get_flag(RFlags::ZERO));
        assert!(result.registers.get_flag(RFlags::SIGN));
    }
    ...
    fn test_or_instruction_zero_result() {
        ...
        let result = execute_instruction(&[0x48, 0x83, 0xC8, 0x00], state).unwrap();
    ...
    fn test_or_instruction_all_ones() {
        ...
        let result = execute_instruction(&[0x48, 0x83, 0xC8, 0xFF], state).unwrap();
    ...
    fn test_out_instruction() {
        ...
        let result = execute_instruction(&[0xEE], state).unwrap();
        
        // OUT instruction should not modify registers (just logs)
        assert_eq!(result.registers.rdx, 0x123456789ABCDE80);
        assert_eq!(result.registers.rax, 0x123456789ABCDE5A);
    }
    ...
    fn test_out_immediate_instruction() {
        ...
        let result = execute_instruction(&[0xE6, 0x80], state).unwrap();
    ...
    fn test_outsb_instruction() {
        ...
        let result = execute_instruction(&[0x6E], state).unwrap();
    ...
    fn test_outsb_instruction_direction_flag() {
        ...
        assert_eq!(result.registers.rsi, 0x0FFF);
        assert_eq!(result.registers.rdx, 0x123456789ABCDE80);
    }
    ...
    fn test_outsw_instruction() {
        ...
        let result = execute_instruction(&[0x66, 0x6F], state).unwrap();
    ...
    fn test_outsw_instruction_direction_flag() {
        ...
        assert_eq!(result.registers.rsi, 0x0FFE);
        assert_eq!(result.registers.rdx, 0x123456789ABCDE80);
    }
    ...
    fn test_outsd_instruction() {
        ...
        let result = execute_instruction(&[0x6F], state).unwrap();
    ...
    fn test_outsd_instruction_direction_flag() {
        ...
        assert_eq!(result.registers.rsi, 0x0FFC);
        assert_eq!(result.registers.rdx, 0x123456789ABCDE80);
    }
    ...
    fn test_orpd_instruction() {
        ...
        let result = execute_instruction(&[0x66, 0x0F, 0x56, 0xC1], state).unwrap();
        
        // ORPD is currently a placeholder, should not crash
        assert!(result.registers.rax == 0);
    }
    ...
    fn test_orps_instruction() {
        ...
        let result = execute_instruction(&[0x0F, 0x56, 0xC1], state).unwrap();
    ...
    fn test_instruction_error_handling() {
        ...
        let result = execute_instruction(&[0x90], state); // NOP instead of OR
    ...
    fn test_or_instruction_parity_flag() {
        ...
        assert_eq!(result.registers.rax & 0xFF, 0xFF);
        assert!(result.registers.get_flag(RFlags::PARITY)); // 0xFF has even parity
    }
    ...
    fn test_or_instruction_combination() {
        ...
        // First OR: RAX | RBX
        let result1 = execute_instruction(&[0x48, 0x09, 0xD8], state).unwrap();
        // OR result: 0x123456789ABCDEF0 | 0xFEDCBA9876543210 = 0xFEFCFEF8FEFCFEF0
        assert_eq!(result1.registers.rax, 0xFEFCFEF8FEFCFEF0);
        
        // Second OR: RAX | RCX
        let result2 = execute_instruction(&[0x48, 0x09, 0xC8], result1).unwrap();
        // OR result: 0xFEFCFEF8FEFCFEF0 | 0x1111111111111111 = 0xFFFDFFF9FFFDFFF1
        assert_eq!(result2.registers.rax, 0xFFFDFFF9FFFDFFF1);
    }
}
