...
fn test_sub_instruction() {
    ...
    let result = execute_instruction(&[0x48, 0x29, 0xC3], &mut state);
...
fn test_shl_instruction() {
    ...
    let result = execute_instruction(&[0x48, 0xC1, 0xE0, 0x02], &mut state);
...
fn test_shl_with_carry() {
    ...
    let result = execute_instruction(&[0x48, 0xD1, 0xE0], &mut state);
...
fn test_shr_instruction() {
    ...
    let result = execute_instruction(&[0x48, 0xC1, 0xE8, 0x02], &mut state);
...
fn test_shr_with_carry() {
    ...
    let result = execute_instruction(&[0x48, 0xD1, 0xE8], &mut state);
...
fn test_sar_instruction() {
    ...
    let result = execute_instruction(&[0x48, 0xD1, 0xF8], &mut state);
...
fn test_stc_instruction() {
    ...
    // STC (0xF9)
    let result = execute_instruction(&[0xF9], &mut state);
    assert!(state.registers.get_flag(RFlags::CARRY));
}
...
fn test_std_instruction() {
    ...
    // STD (0xFD)
    let result = execute_instruction(&[0xFD], &mut state);
    assert!(state.registers.get_flag(RFlags::DIRECTION));
}
...
fn test_sti_instruction() {
    ...
    // STI (0xFB)
    let result = execute_instruction(&[0xFB], &mut state);
    assert!(state.registers.get_flag(RFlags::INTERRUPT));
}
...
fn test_salc_instruction() {
    ...
    let result = execute_instruction(&[0xD6], &mut state);
...
fn test_sbb_instruction() {
    ...
    let result = execute_instruction(&[0x48, 0x19, 0xC3], &mut state);
...
fn test_stosb_instruction() {
    ...
    // STOSB (0xAA)
    let result = execute_instruction(&[0xAA], &mut state);
    assert_eq!(state.registers.rdi, 0x1001); // RDI incremented by 1
    assert_eq!(state.read_u8(0x1000).unwrap(), 0xF0); // AL (0xF0) stored at [RDI]
}
...
fn test_stosw_instruction() {
    ...
    // STOSW (0x66 0xAB)
    let result = execute_instruction(&[0x66, 0xAB], &mut state);
    assert_eq!(state.registers.rdi, 0x1002); // RDI incremented by 2
    assert_eq!(state.read_u16(0x1000).unwrap(), 0xDEF0); // AX (0xDEF0) stored at [RDI]
}
...
fn test_stosd_instruction() {
    ...
    // STOSD (0xAB)
    let result = execute_instruction(&[0xAB], &mut state);
    assert_eq!(state.registers.rdi, 0x1004); // RDI incremented by 4
    assert_eq!(state.read_u32(0x1000).unwrap(), 0x9ABCDEF0); // EAX (0x9ABCDEF0) stored at [RDI]
}
...
fn test_stosq_instruction() {
    ...
    // STOSQ (0x48 0xAB)
    let result = execute_instruction(&[0x48, 0xAB], &mut state);
    assert_eq!(state.registers.rdi, 0x1008); // RDI incremented by 8
    assert_eq!(state.read_u64(0x1000).unwrap(), 0x123456789ABCDEF0); // RAX stored at [RDI]
}
...
fn test_scasb_instruction() {
    ...
    // SCASB (0xAE)
    let result = execute_instruction(&[0xAE], &mut state);
    assert_eq!(state.registers.rdi, 0x1001); // RDI incremented by 1
...
fn test_scasw_instruction() {
    ...
    // SCASW (0x66 0xAF)
    let result = execute_instruction(&[0x66, 0xAF], &mut state);
    assert_eq!(state.registers.rdi, 0x1002); // RDI incremented by 2
...
fn test_scasd_instruction() {
    ...
    // SCASD (0xAF)
    let result = execute_instruction(&[0xAF], &mut state);
    assert_eq!(state.registers.rdi, 0x1004); // RDI incremented by 4
...
fn test_scasq_instruction() {
    ...
    // SCASQ (0x48 0xAF)
    let result = execute_instruction(&[0x48, 0xAF], &mut state);
    assert_eq!(state.registers.rdi, 0x1008); // RDI incremented by 8
...
fn test_sahf_instruction() {
    ...
    // SAHF (0x9E)
    let result = execute_instruction(&[0x9E], &mut state);
...
fn test_syscall_instruction() {
    ...
    // SYSCALL (0x0F 0x05)
    let result = execute_instruction(&[0x0F, 0x05], &mut state);
...
fn test_sysret_instruction() {
    ...
    // SYSRET (0x0F 0x07)
    let result = execute_instruction(&[0x0F, 0x07], &mut state);
...
fn test_shift_count_mask() {
    ...
    let result = execute_instruction(&[0x48, 0xC1, 0xE0, 0x41], &mut state);
...
fn test_string_operations_direction_flag() {
    ...
    let result = execute_instruction(&[0xAA], &mut state);
    assert_eq!(state.registers.rdi, 0xFFF); // RDI decremented by 1
    assert_eq!(state.read_u8(0x1000).unwrap(), 0xF0); // AL stored at original RDI
}
