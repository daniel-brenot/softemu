...
fn main() -> Result<()> {
    ...
    // Test 10: Large page support (2MB pages)
    println!("\n=== Test 10: Large page support ===");
    let large_virtual = 0x100000; // 1MB aligned
    let large_physical = 0x200000; // 2MB aligned
    let large_flags = PageTableEntryFlags::PRESENT | PageTableEntryFlags::WRITABLE | PageTableEntryFlags::USER_ACCESSIBLE | PageTableEntryFlags::PAGE_SIZE;
    
    memory_manager.create_page_table_entry(large_virtual, large_physical, large_flags)?;
    
    // Test translation at different offsets within the large page
    for offset in [0x0, 0x1000, 0x100000, 0x1FF000].iter() {
        let test_virtual = large_virtual + offset;
        let expected_physical = large_physical + offset;
        let translated = memory_manager.translate_address_with_access(test_virtual, false, false, false)?;
        println!("Large page: 0x{:x} -> 0x{:x} (expected 0x{:x})", test_virtual, translated, expected_physical);
        assert_eq!(expected_physical, translated);
    }
